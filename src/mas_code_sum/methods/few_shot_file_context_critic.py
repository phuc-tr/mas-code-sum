...
def _score(candidate: str, example_refs: list[str], outline_refs: list[str]) -> float:
    import statistics
    top_examples = example_refs[:3]
    example_score = statistics.mean(_bleu([ref], candidate)[0] for ref in top_examples)
    if not outline_refs:
        return example_score
    scored = sorted((_bleu([ref], candidate)[0] for ref in outline_refs), reverse=True)
    top_outline = scored[:3]
    outline_score = statistics.mean(top_outline)
...
def _pick_best(initial: str, refined: str, example_refs: list[str], outline_refs: list[str]) -> tuple[str, str]:
    ...
    if _score(refined, example_refs, outline_refs) > _score(initial, example_refs, outline_refs):
...
class FewShotFileContextCriticSummarizer(BaseSummarizer):
    ...
    Stage 1 (summarizer): completions API via Featherless + Meta-Llama-3.1-8B (base).
    ...
    def __init__(
        ...
        self._client, self._async_client = make_clients("featherless")
        self._critic_client, self._critic_async_client = make_clients(backend)
            ...
            async def _one(code, lang, proj, path, url, gt):
                async with sem:
                    return await _call_with_rate_limit_retry(
                        lambda: self._summarize_one(code, lang, proj, path, url, gt)
                    )
