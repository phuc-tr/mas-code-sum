...
base = ParserBase(parser_path="parser/languages.so",language="java")
...
##types, tokens=base.get_tokens_with_node_type(code.encode(),root)
...
with open("test_id_type3.txt","w") as f:
    for i in range(len(test_json)):
        ...
        if len(access)>0:    
            f.write("Identifier to access field/array: ")
            f.write(str(access)+"\n")        
